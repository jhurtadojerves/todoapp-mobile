...
describe('DeleteSprintUseCase', () => {
  ...
  it('should call sprintRepository.deleteSprint with the given board id and sprint id', async () => {
    mockSprintRepository.deleteSprint.mockResolvedValue(undefined);

    await useCase.execute(1, 1);

    expect(mockSprintRepository.deleteSprint).toHaveBeenCalledWith(1, 1);
  });
