...
describe('Board members flow (integration)', () => {
  ...
  it('should list the board members from a paginated response', async () => {
    ...
    apiMock.onAny().reply(200, { count: 2, next: null, previous: null, results: [owner, member] });

    const result = await getMembersUseCase.execute(1, 1);
  ...
  it('should invite a member by email and return the created membership', async () => {
    ...
    apiMock.onAny().reply(201, member);

    const result = await addMemberUseCase.execute(1, { email: 'jane@example.com' });
  ...
  it('should surface a validation error when the invited email does not exist', async () => {
    ...
    await expect(
      addMemberUseCase.execute(1, { email: 'ghost@example.com' })
  ...
  it('should remove a member', async () => {
    ...
    await expect(removeMemberUseCase.execute(1, 2)).resolves.toBeUndefined();
  });
  ...
  it('should surface a 403 as a permission error when a non-owner tries to remove a member', async () => {
    ...
    await expect(removeMemberUseCase.execute(1, 2)).rejects.toThrow(
