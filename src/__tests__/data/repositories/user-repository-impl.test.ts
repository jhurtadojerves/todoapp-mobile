...
describe('UserRepositoryImpl', () => {
  ...
  it('should delegate fetchUsers to the data source', async () => {
    ...
    const result = await repository.fetchUsers(1);

    expect(mockDataSource.fetchUsers).toHaveBeenCalledWith(1);
  ...
  it('should propagate errors from the data source', async () => {
    ...
    await expect(repository.fetchUsers(1)).rejects.toThrow('Network error');
  });
});
